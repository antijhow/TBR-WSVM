...
theta.true = c(theta.true,array(0,16))
p = 4
q = 2
...
for (k in 1:loop)
{
	...
	z.l = z.b
	add.x = matrix(rnorm(n*16),n,16) # add more redundant covariates
	z.b = cbind(z.b,add.x)
	y = y[(q+1):(n+q)]
	...
	theta.b.matrix = final.est/sqrt(sum(final.est^2))
	temp = z.b%*%theta.b.matrix
	test.h = which(temp>=0)
	test.g = which(temp<0)
	true = z.b%*%theta.true
