...
sq.res = function(z.l,z.b,y,set.h,set.g) #z.b design matrix of threshold funciton with intercept
{
	n = dim(z.l)[1]
	lm.dim = dim(z.l)[2]+1
...
total.sse = function(z.l,z.b,y,set.h,set.g) #z.b design matrix of threshold funciton with intercept
{
	n = dim(z.l)[1]
	lm.dim = dim(z.l)[2]+1
	if (length(set.h)<=lm.dim || length(set.g)<=lm.dim)
...
MCMC.series = function(z.l,z.b,y,niter,max,min,q) # max and min can be arrays
{
	d = dim(z.b)[2]
	n = length(y)
	gamma = matrix(0,d,niter)
	gamma[q,] = 1
	gamma[-q,1] = runif(d-1,min,max)
	for (i in 2:niter) 
	{
		currentx = gamma[,i-1]
...
MCMC = function(z.l,z.b,y,niter,niter1,max,min,q)
{
	gamma = MCMC.series(z.l,z.b,y,niter,max,min,q)
...
threshold.wsvm = function(z.l,z.b,y,initial.h,initial.g,cost=10,upper=50,method="linear")
{
	...
	{	
		...
		pred.y = predict(bound.wsvm,data[,-1])
		pred.y = as.numeric(pred.y)
		set.h = which(pred.y == 1)
		set.g = which(pred.y == 2)	
		loss.temp = total.sse(z.l,z.b,y,set.h,set.g)
		...
		if (loss.min > loss.temp && test !=n)
		{
			...
			count = count +1
			temp = initial.set.kmeans(z.l,z.b,y,sqrt(n))
