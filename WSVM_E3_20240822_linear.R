...
p = 4
q = 2
theta.b.matrix = theta.b.mc.matrix = matrix(0,loop,p+q+1)

acc.rate = acc.rate.mc = array(NA,loop)
balance = array(0,loop)
mse = mse.mc = array(0,loop)
...
for (k in 1:loop)
{
	...
	final.est = final.est/final.est[2]
	temp = MCMC(z.l,z.b,y,niter=2000,niter1=100,max=2,min=-2,q=2,initial=final.est)
	theta.b.mc.matrix[k,] = temp/sqrt(sum(temp^2))
	...
	temp = z.b%*%theta.b.mc.matrix[k,] 
	test.h = which(temp>=0)
	test.g = which(temp<0)
	true = z.b%*%theta.true
	acc.rate.mc[k] = length(which(temp*true >=0))/n
	if (acc.rate.mc[k]<0.5) 
	{
	  theta.b.mc.matrix[k,]= -theta.b.mc.matrix[k,]
	  acc.rate.mc[k] = 1-acc.rate.mc[k]
	}
	mse.mc[k] = total.sse(z.l,y,test.h,test.g)/n
	print(k)
}
...
apply(theta.b.mc.matrix,2,mean)
apply(theta.b.mc.matrix,2,sd)
theta.true
mean(acc.rate)
sd(acc.rate)
mean(acc.rate.mc)
